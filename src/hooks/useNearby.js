...
async function buildRows(routeMap, lat, lng, dist, transportSettings) {
  ...
  // 車費（KMB / LWB / joint，最多等 4 秒）
  // LWB 使用與 KMB 相同的車費 API
    ...
    try {
      ...
      etaResults.forEach((res, i) => {
        ...
        (res.data || []).forEach(e => {
          ...
          const ts = new Date(e.eta).getTime();
          if (ts < now - 30000) return;
          // ★ 修正：透過 e.co 區分龍運 (LWB) 與九巴 (KMB)
          const coType = e.co === 'LWB' ? 'lwb' : 'kmb';
          ...
          if (!routeMap.has(key)) {
            routeMap.set(key, {
              ...
              companyType: coType,
              etasWithType: [{ ts, type: coType }],
              fare: null,
            });
          } else {
            const ex = routeMap.get(key);
            if (ex.etasWithType.length < 3 && !ex.etasWithType.find(x => x.ts === ts))
              ex.etasWithType.push({ ts, type: coType });
          }
        });
      });
          ...
          if (ctbRows.length > 0) {
            ctbRows.forEach(r => {
              ...
              if (matchKey) {
                ...
                if (ex.companyType === 'kmb' || ex.companyType === 'lwb') ex.companyType = 'joint';
