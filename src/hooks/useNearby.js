    ...
    try {
      ...
      etaResults.forEach((res, i) => {
        ...
        (res.data || []).forEach(e => {
          ...
          const ts = new Date(e.eta).getTime();
          if (ts < now - 30000) return;
          const coType = e.co === 'LWB' ? 'lwb' : 'kmb';                          // ★ 修正：讀 e.co 區分龍運
          ...
          if (!routeMap.has(key)) {
            routeMap.set(key, {
              ...
              companyType: coType, etasWithType: [{ ts, type: coType }], fare: null, // ★ 用 coType
            });
          } else {
            const ex = routeMap.get(key);
            if (ex.etasWithType.length < 3 && !ex.etasWithType.find(x => x.ts === ts))
              ex.etasWithType.push({ ts, type: coType });                           // ★ 用 coType
          }
        });
      });
