...
export default function SearchPage({ isActive }) {
  ...
  const [targetPid, setTargetPid] = useState(() => activePid !== NEARBY_PID ? activePid : null);
  ...
  const addStop = (stop) => {
    ...
    if (favList.some(f => f.stopId === stop.stop && f.route === selectedRoute.route)) { showToast('⚠️ 此站已加入'); return; }
    ...
    return (
      <div className="page" id="page-search" style={isActive ? { display: 'flex' } : {}}>
        <div style={{ flexShrink: 0, padding: '12px 12px 10px', background: 'var(--bg2)', borderBottom: '1px solid var(--bdr)' }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--bright)' }}>{selectedRoute.route} 往 {selectedRoute.dest_tc}</div>
              ...
              {needPicker
                ? profiles.map(p => (
                  <button key={p.id} onClick={() => setTargetPid(p.id)}
                    style={{ padding: '4px 10px', borderRadius: 8, fontSize: 11, cursor: 'pointer', fontFamily: 'var(--sans)', background: targetPid === p.id ? 'var(--amb-bg)' : 'var(--bg3)', border: `1px solid ${targetPid === p.id ? 'var(--amb-bdr)' : 'var(--bdr2)'}`, color: targetPid === p.id ? 'var(--amb2)' : 'var(--mid)' }}>{p.name}</button>
                ))
                : <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--amb2)' }}>{profiles.find(p => p.id === activePid)?.name}</span>
              }
            </div>
          )}
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px 16px', scrollbarWidth: 'thin' }}>
          {stopsLoading ? <Spinner /> : (!stops || stops.length === 0
      ...
      <div style={{ flexShrink: 0, padding: '12px 12px 8px', background: 'var(--bg2)', borderBottom: '1px solid var(--bdr)' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          <input className="d-input" value={query} placeholder="路線號碼 / 起終點站（如 40X、荃灣）"
            onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && doSearch()} style={{ fontFamily: 'var(--sans)' }} />
