...
export default function HomePage({ openDrawer, showToast, isActive }) {
  ...
  // ★ App 從背景返回前台時重新整理（visibilitychange）
  // 用 ref 存最新的 refresh 邏輯，避免 stale closure 問題
  const refreshRef = useRef(null);
  refreshRef.current = useCallback(() => {
    if (isNearby && gpsCoords) nearbyHook.load(gpsCoords.lat, gpsCoords.lng, nearbyDist);
    else _refreshFavs();
  // eslint-disable-next-line
  }, [isNearby, gpsCoords, nearbyDist]);
  ...
  const _refreshFavs = useCallback(async () => {
    ...
    setFavRows(results);
    // 車費背景更新（KMB / LWB / joint 均使用 KMB 車費 API）
    results.forEach(async (r, i) => {
      if (r.companyType !== 'kmb' && r.companyType !== 'lwb' && r.companyType !== 'joint') return;
