...
export default function HomePage({ openDrawer, closeDrawer, showToast }) {
  const { activePid, setActivePid, profiles, nearbyDist, setNearbyDist,
    gpsCoords, saveGps, selectedStn, setSelectedStn, transportSettings,
    favs: ctxFavs, setFavs, reloadFavs } = useApp();
  ...
  const { getCurrentPosition, getHighAccuracy, checkPermission } = useGeolocation();
  ...
  useEffect(() => {
    loadWeather();
    if (gpsCoords && isNearby) nearbyHook.load(gpsCoords.lat, gpsCoords.lng, nearbyDist);
    else if (isNearby) {
    ...
    const etaT = setInterval(() => { if (isNearby && gpsCoords) nearbyHook.load(gpsCoords.lat, gpsCoords.lng, nearbyDist); else _refreshFavs(); }, 30000);
    return () => { clearInterval(weatherT); clearInterval(etaT); };
  // eslint-disable-next-line
  }, []);

  // Re-load nearby when dist changes
  useEffect(() => {
  ...
  const _refreshFavs = useCallback(async () => {
    ...
    const results = await Promise.all(favList.map(async fav => {
      try {
        ...
        const etas = (d.data || []).filter(e => e.eta).slice(0, 3).map(e => new Date(e.eta).getTime()).filter(ts => ts > now - 30000);
        return { ...fav, etasWithType: etas.map(ts => ({ ts, type: fav.type || 'kmb' })), companyType: fav.type || 'kmb', fare: null };
      } catch { return { ...fav, etasWithType: [], companyType: fav.type || 'kmb', fare: null }; }
    }));
    setFavRows(results);
    // Fetch fares in background
  ...
  const doRefresh = async () => {
    ...
    await Promise.all([
      loadWeather(),
      isNearby && gpsCoords ? nearbyHook.load(gpsCoords.lat, gpsCoords.lng, nearbyDist) : _refreshFavs(),
    ]);
    setRefreshing(false);
  };
  ...
  const setDist = (m) => {
    setNearbyDist(m);
    if (gpsCoords) nearbyHook.load(gpsCoords.lat, gpsCoords.lng, m);
  };

  // ── Nearby bus list content ───────────────────────────────
        ...
        return nearbyHook.rows.map((row, i) => (
          <BusCard key={`${row.route}_${row.stopId}_${i}`} row={row} idx={i}
            onClick={row.companyType !== 'mtr' ? () => {
              ...
              openDrawer(`${row.route} 路線詳情`, null); // detail drawer handled in App
            } : undefined}
          />
        ));
  ...
  const rows = isNearby ? nearbyHook.rows : favRows;
  ...
  return (
    <div className="page active" id="page-home">
      ...
      {/* Profiles bar rendered in App.jsx */}
      ...
      <div className="bus-sec">
        ...
        {isNearby && (
          <div className="dist-row">
            ...
            {STD_DISTS.map(m => (
              <button key={m} className={`dist-pill${nearbyDist === m ? ' active' : ''}`} onClick={() => setDist(m)}>
                {distLabel(m)}
              </button>
            ))}
            <button className={`dist-pill${!isStdDist ? ' active' : ''}`} onClick={() => {
              const cur = DIST_STEPS.reduce((b, v, i) => Math.abs(v - nearbyDist) < Math.abs(DIST_STEPS[b] - nearbyDist) ? i : b, 0);
        ...
        <div className="bus-list" style={{ display: mapView ? 'none' : undefined }}>
          {isNearby ? renderNearbyContent() : (
            favRows.length === 0 ? (
              ...
              <BusCard key={`${row.route}_${row.stopId}_${i}`} row={row} idx={i}
                onRemove={removeFav}
                onClick={() => { incrementRouteUsage(row.route, row.companyType); openDrawer(`${row.route} 路線詳情`, 'bus-detail'); }}
              />
            ))
          )}
        </div>
        ...
        {isNearby && mapView && (
          <div id="nearby-map" ref={mapRef} style={{ flex: 1, minHeight: 0, position: 'relative' }} />
        )}
      </div>
        ...
        <div style={{ position: 'fixed', bottom: 'calc(var(--nav-h) + 8px)', left: 0, right: 0, zIndex: 40, display: 'flex', justifyContent: 'center', padding: '0 12px' }}>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--bdr2)', borderRadius: 14, padding: '14px 16px 12px', width: '100%', maxWidth: 480, boxShadow: '0 6px 28px rgba(0,0,0,.6)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button onClick={() => setShowSlider(false)} style={{ background: 'var(--bg3)', border: '1px solid var(--bdr2)', color: 'var(--mid)', borderRadius: 8, padding: '7px 16px', fontSize: 13, cursor: 'pointer' }}>取消</button>
              <button onClick={() => { setDist(DIST_STEPS[sliderIdx]); setShowSlider(false); }} style={{ background: 'var(--amb)', color: '#000', border: 'none', borderRadius: 8, padding: '7px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>確定</button>
            </div>
          </div>
        </div>
      )}
